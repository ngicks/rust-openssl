...
foreign_type_and_impl_send_sync! (
    ...
    fn clone = ffi::TS_REQ_dup;

    /// [`RFC 3161`]: https://tools.ietf.org/html/rfc3161
    pub struct TsReq;
    /// Reference to [`TsReq`]
    ///
    /// [`TsReq`]:struct.TsReq.html
    pub struct TsReqRef;
);
...
foreign_type_and_impl_send_sync! (
    ...
    fn clone = ffi::TS_RESP_dup;

    /// [`RFC 3161`]: https://tools.ietf.org/html/rfc3161
    pub struct TsResp;
    /// Reference to [`TsResp`]
    ///
    /// [`TsResp`]:struct.TsReq.html
...
impl TsTstInfoRef {
    ...
    pub fn get_exts(&self) -> Option<&StackRef<X509Extension>> {
        unsafe {
...
mod tests {
    ...
    fn test_from_cms() {
        let tst_info = {
            ...
            TsTstInfo::from_der(cms.get_content().unwrap().unwrap().as_slice()).unwrap()
        };
    ...
    fn test_get(tst_info: &TsTstInfo) {
        assert_eq!(tst_info.get_version(), 1);
        assert_eq!(
            // Basic Time-Stamping policy
            "1.3.6.1.4.1.38064.1.3.6.1",
            tst_info.get_policy_id().to_string()
        );
        assert_eq!(
            "4828232355673314455",
            tst_info.get_serial().to_bn().unwrap().to_string()
        );
        assert_eq!(
            "20211214111051Z",
            tst_info.get_time().as_utf8().unwrap().to_string()
        );
        assert!(!tst_info.get_ordering());
        assert_eq!(
            "-1578146648116833895",
            tst_info.get_nonce().unwrap().to_bn().unwrap().to_string()
        );
        assert!(tst_info.get_tsa().is_none());
    }
    ...
    fn test_accuracy(tst_info: &TsTstInfo) {
        ...
        assert_eq!("1", acc.get_seconds().unwrap().to_bn().unwrap().to_string());
    ...
    fn test_extensions(tst_info: &TsTstInfo) {
        ...
        assert!(tst_info.get_exts().is_none());
    }
}
